    ...
    def check_drivers(self):
        ...
        for p in self._pins:
            if not p.is_hiz():
                if d:
                    warn(
                        "Warning: Multiple drivers on {}: {} {}".format(
                            self.name(), d.fullname(), p.fullname()
                        ),
                        dedup=True,
                    )
                d = p
...
class SignalView:
    ...
    def name(self):
        if self == self._signal._view:
            ...
            return "{}:{}".format(
                self._signal._component.name(), ",".join(p.name() for p in self._pins)
            )
    ...
    def __ilshift__(self, v):
        if v is None:
            ...
            if v < 0:
                ...
                raise Exception(
                    "invalid value -- {} > {} for {}".format(v, self._max, self.name())
                )
            for p in self._pins:
                p <<= v & 1
                v >>= 1
    ...
    def __iadd__(self, other):
        ...
        if len(self._pins) != len(other._pins):
            raise Exception(
                "Mismatched signal widths: {} and {}".format(self.name(), other.name())
            )
...
class Signal:
    ...
    def name(self):
        if len(self) == 1:
            ...
            return "{}:{}[0..{}]".format(
                self._component.name(), self._name, len(self) - 1
            )
...
class Ram(Component):
    ...
    def update(self, signal):
        if self.we.had_edge(0, 1):
            trace(
                "write: ram[0x{:04x}] = 0x{:02x}".format(
                    self.addr.value(), self.data.value()
                )
            )
    ...
    def stdout(self):
        ...
        for i in range(0, len(self.ram), 16):
            ...
            print(
                "{:04x}: {}".format(
                    i, " ".join("{:02x}".format(b) for b in self.ram[i : i + 16])
                )
            )
...
class Display(Component):
    ...
    def update(self, signal):
        if self.last != self.data.value():
            ...
            print(
                "{{}} {{:0{}b}}".format(len(self.data)).format(self.name(), self.last)
            )
...
class MemoryDevice(Component):
    ...
    def update(self, signal):
        if (
            self.addr.value() >= self.base_addr
            and self.addr.value() < self.base_addr + self.size
        ):
            self.oe_out <<= 0
            self.we_out <<= 0
            if self.we.had_edge(0, 1):
...
class MemDisplay(MemoryDevice):
    def __init__(self, addr_width=16, data_width=8, base_addr=0):
        super().__init__(
            "mem display", base_addr, 2, addr_width=addr_width, data_width=data_width
        )
        self.v = 0
        self.trigger = 0
...
class RNG(MemoryDevice):
    def __init__(self, addr_width=16, data_width=8, base_addr=0):
        super().__init__(
            "rng", base_addr, 1, addr_width=addr_width, data_width=data_width
        )
