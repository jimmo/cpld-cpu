...
class Assembler:
    ...
    def __exit__(self, a, b, c):
        for p in self._pages.values():
            ...
            for name, offset_num, offset_sta in p._fixups:
                self.log(
                    "fixup: {}::{} --> 0x{:04x} 0x{:04x}".format(
                        p._name, name, offset_num, offset_sta
                    )
                )
        ...
        for l in self._labels.values():
            ...
            for page, offset, is_jump in l._fixups:
                if is_jump:
                    ...
                    if (
                        page._target == l._page._target
                        and page._num != l._page._num
                        and not l._register
                    ):
                        raise ValueError(
                            f"Referecing label {l._name} from a different page in the same page"
                        )
    ...
    def jcs(self, label):
        ...
        self.write_instr(
            Assembler.PREFIX_JCC | (self._page._target << 12) | (self._offset + 4)
        )
        self._indent += 1
        self.jcc(label)
        self._indent -= 1
    ...
    def jz(self, label):
        ...
        self.write_instr(
            Assembler.PREFIX_JNZ | (self._page._target << 12) | (self._offset + 4)
        )
        self._indent += 1
        self.jcc(label)
        self._indent -= 1
    ...
    def hlt(self):
        self.log("  hlt")
        self.write_instr(
            Assembler.PREFIX_JCC | (self._page._target << 12) | self._offset
        )
        self.write_instr(
            Assembler.PREFIX_JCC | (self._page._target << 12) | self._offset
        )
