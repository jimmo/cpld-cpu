import sys
from sim import Component, Signal, NotifySignal, Net, Register, IORegister, IncRegister, Clock, Ram, Rom, Power, MemDisplay
...
class Decoder(Component):
  ...
  def update(self, signal):
    ...
    self.ram_oe <<= self.state <= 3 or (self.instr.value() in (0b00, 0b01,) and self.state <= 5)
    ...
    # alu
    self.alu_we <<= self.instr.value() in (0b00, 0b01,) and self.state == 5
    self.alu_oe <<= self.instr.value() in (0b00, 0b01,) and self.state in (5, 6,)
    self.a_we <<= self.instr.value() in (0b00, 0b01,) and self.state == 6
    self.fn <<= self.instr.value() & 1

    # sta
    self.a_oe <<= self.instr.value() == 0b10 and self.state in (5, 6,)
    self.ram_we <<= self.instr.value() == 0b10 and self.state == 6

    # jcc c=0
    self.pc_we <<= self.instr.value() == 0b11 and self.carry.value() == 0 and self.state == 5
    # jcc c=1
    self.a_cc<<= self.instr.value() == 0b11 and self.carry.value() != 0 and self.state == 5
